...
Editing Command: <%= command.name ? command.name : (command.id ? "COMMAND_ID_"+ command.id : "New Command")%>
</div>
</div>
...
<textarea class="form-control " rows="15" name="code" id="code" placeholder="The code of the command" required><%= command.code ? command.code : "" %></textarea>
</div>
<div id="extraProperty">
<% Object.keys(command).filter( f => !(["callback", "code", "name", "id"].includes(f))).forEach(f => { %>
<div id="<%= f %>" class="mb-3">

<label for="<%= f %>" class="form-label display-6"><i class="hover-click bi bi-x-circle" style="color:red;" onclick="DeleteProperty('<%= f %>')"></i> Command <%= f %>:</label>
...
<button class="btn btn-primary form-button" type="submit">Save Command</button>
