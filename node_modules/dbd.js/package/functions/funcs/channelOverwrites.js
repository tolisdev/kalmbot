module.exports = async d => {
    ...
    if (!channel) return d.error(`❌ Invalid channel ID in \`$channelOverwrites${data.total}\``) 
    
    const placer = channel.permissionOverwrites.array().map(overwrites => {
        return column 
        .replace(/{allow}/g, overwrites.allow.toArray().goof())
        .replace(/{deny}/g, overwrites.deny.toArray().goof())
        .replace(/{mention}/g, overwrites.type === "role" ? `<@&${overwrites.id}>` : `<@${overwrites.id}>`) 
