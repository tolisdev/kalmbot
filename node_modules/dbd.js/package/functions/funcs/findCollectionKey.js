module.exports = async d => {
    ...
    const inside = code.split("$findCollectionKey[") [r].split("]")[0] 
    
    const [cname, value, accurate = "yes"] = inside.split(";") 
    
    const collection = d.client.collections[cname.addBrackets()] 
    
    if (!collection) return d.error(`❌ Invalid collection name in \`$findCollectionKey[${inside}]\``) 
    ...
    return {
        code: code.replaceLast(`$findCollectionKey[${inside}]`, key ? key.deleteBrackets() : "")
    }
}