module.exports = async d => {
    ...
    const inside = code.split("$getEmbed[")[r].split("]")[0]
    ...
    if (!channel) return d.error(`:x: Invalid channel ID in \`$getEmbed[${inside}]\``)
    ...
    const opt = eval(options ? `msg.embeds[0].${options.replace("? ", "? msg.embeds[0].")}` : `msg.embeds[0].${option.addBrackets()}`)
