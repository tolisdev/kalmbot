...
class CreateEvent {
  constructor(client, EventEmitter) {
    ...
    if (this.event._eventsCount === undefined) throw new Error("Data is not an Event Emitter")
    this.commands = []
  ...
  command(...args) {
    ...
    if (isNaN(parseInt(d.channel))) throw new TypeError("Event Command needs a Snowflake Channel ID")

    if (!d.listen) throw new TypeError("Event Command needs a 'listen' to listen to an event")
