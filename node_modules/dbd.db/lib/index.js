...
async function deprecated(dbName, colName) {
	...
	await new Bluebird(async (resolve, reject) => {
		...
		dataStream.once('end', async () => {
			...
			data = await Bison.splitArray(data.split(' '), 64)
		...
		if (exist['meta']) {
			...
			metaStream.once('end', async () => {
				...
				meta = await Bison.splitArray(meta.split(' '), 64)
				...
				const metaJSON = await new Bluebird(async resolveJSON => {
					try {
		...
		if (exist['ttl']) {
			...
			ttlStream.once('end', async () => {
				...
				ttl = await Bison.splitArray(ttl.split(' '), 64)
